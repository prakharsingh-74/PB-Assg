  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <main className="max-w-7xl mx-auto px-6 md:px-12 pt-8">
        ...
        {/* Search and Sort Row - Adjusted to match screenshot */}
        <div className="mt-6 flex flex-col xl:flex-row items-center gap-6">
            {/* Search Box */}
            <div className="flex-1 w-full relative">
                <div className="flex items-center gap-3 px-4 py-3 bg-white border border-slate-200 rounded-md focus-within:border-primary transition-all">
                    <Search className="w-4 h-4 text-primary shrink-0" />
                    <input 
                        type="text"
                        placeholder="Search"
                        className="flex-1 bg-transparent border-none outline-none text-[15px] font-medium text-slate-700 placeholder:text-slate-300"
                        value={search}
            ...
            <div className="flex items-center gap-4 shrink-0 w-full xl:w-auto">
                <span className="text-[17px] font-bold text-primary">Sort by:</span>
                <div className="flex items-center gap-3">
                    <button 
                        onClick={() => setSortBy(sortBy === 'patient_name' ? 'patient_id' : 'patient_name')}
                        className="px-4 py-2 border border-slate-200 rounded-md text-[14px] font-medium text-slate-600 flex items-center gap-4 hover:bg-slate-50 transition-all min-w-[120px] justify-between"
                    >
                        option 1
                        <span className="text-[12px] text-slate-400">&uarr;&darr;</span>
                    </button>
                    <button 
        ...
        <div className="mt-4 flex flex-wrap gap-3">
            {MEDICAL_ISSUES.slice(0, 4).map(issue => (
                <div 
                    ...
                    className={`flex items-center gap-2 px-4 py-2.5 border rounded-md text-[15px] font-medium transition-all cursor-pointer ${selectedIssues.includes(issue) ? 'bg-primary/5 border-primary text-primary' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'}`}
                >
                    {issue}
                    <X className={`w-4 h-4 ${selectedIssues.includes(issue) ? 'text-primary' : 'text-slate-400'}`} />
                </div>
            ))}
        </div>
        ...
        <div className="mt-4 flex justify-end">
            <button className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-md text-[13px] font-medium text-slate-600 hover:bg-slate-50 transition-all shadow-sm">
        ...
        <div className="mt-16 mb-20 flex justify-center">
          <div className="flex items-center gap-4">
            <button 
              ...
              className="px-4 py-2 border border-slate-200 rounded-md flex items-center gap-2 text-[13px] font-medium text-slate-500 hover:bg-slate-50 disabled:opacity-40 transition-all bg-white"
            >
              &lt; Previous
            </button>
               ...
               {totalPages > 1 && Array.from({ length: Math.min(7, totalPages) }).map((_, i) => {
                 ...
                 return (
                   <button
                     ...
                     className={`w-8 h-8 rounded-md text-[13px] font-bold transition-all ${page === pageNum ? 'bg-primary text-white' : 'text-slate-400 hover:text-slate-600'}`}
                   >
